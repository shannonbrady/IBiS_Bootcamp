...
dark <- starwars %>% #take the starwars data frame, and then
  ...
  dplyr::group_by(   ) %>% #group the data by species, and then
  dplyr::summarize(average_height =    ) #summarize the data where average_height is the mean of heights (within a species).
